...
if __name__ == "__main__":
    ...
    clf = clf.fit(train_X[:50000], train_y[:50000])
    ...
    def predict_proba_class_1(X):
        return clf.predict_proba(X)[:, 1]  # select class 1 probabilities
...
# compute the SHAP values
    shap_values = explainer.shap_values(test_X[:500],nsamples = 1000)

# visualize the explanation
    #shap.summary_plot(shap_values, test_X[:1000],feature_names = feature_names)
    shap.force_plot(
        explainer.expected_value, shap_values[2], test_X[2],
        feature_names=feature_names,
        matplotlib=True, show=False
    )
    plt.savefig("./plots/" + "SVM_SHAP_1.jpg", format='jpg')