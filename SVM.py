...
def compute_all_metrics(pred_y, labels):
    ...
    ax.set_title('Precision, Recall, F1 Score and Accuracy of Support Vector Machine')
    ax.set_ylabel('Scores')
    plt.tight_layout()
    plt.savefig("./plots/" + "Support Vector Machine_metrics.jpg",format='jpg')
...
if __name__ == "__main__":
    ...
    features_df = pd.DataFrame(data={'feature': feature_names, 'importance': importance})
    ...
    plt.ylabel('Feature')
    plt.title('Top 7 Important Features SVM')
    ...
    for bar in bars:
        plt.text(bar.get_width(), bar.get_y() + bar.get_height()/2, 
                 ...
                 va='center', ha='left')
    plt.tight_layout()
    plt.savefig("./plots/" + "SVM_feature.jpg",format='jpg')
