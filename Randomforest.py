...
def compute_all_metrics(pred_y, labels):
    ...
    ax.set_title('Precision, Recall, F1 Score and Accuracy of Random Forest')

    ax.set_ylabel('Scores')
    plt.tight_layout()
    plt.savefig("./plots/" + "RandomForest_metrics.jpg",format='jpg')
...
if __name__ == "__main__":
    ...
    plt.ylabel('Feature')
    plt.title('Top 7 Important Features for RandomForest')
    ...
    for bar in bars:
        plt.text(bar.get_width(), bar.get_y() + bar.get_height()/2, 
                 ...
                 va='center', ha='left')
    plt.tight_layout()
    plt.savefig("./plots/" + "RandomForest_feature.jpg",format='jpg')
